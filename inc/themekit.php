...
class dcc_2015_Themekit {
	...
	private function loader() {
		...
		add_filter( 'the_content_more_link', array( $this, 'excerpt_read_more' ) );
		add_filter( 'mce_buttons_2', array( $this, 'add_editor_buttons' ) );
		add_action( 'dcc_2015_breadcrumbs', array( $this, 'breadcrumbs' ) );
		add_action( 'category_select', array( $this, 'category_select' ) );
		add_filter( 'wpseo_breadcrumb_links', array( $this, 'author_archives' ) );
	...
	public function more_setup() {
		...
		register_nav_menus( array(
			'social' => esc_html__( 'Social Links', 'dcc-2015' )
		) );
	...
	public function admin_scripts_and_styles() {
		...
		//wp_enqueue_style( 'dcc-2015-admin', get_stylesheet_directory_uri() . '/admin.css' );
...
<!-- Google Tag Manager -->
<noscript><iframe src="//www.googletagmanager.com/ns.html?id=GTM-KXQ5KL"
height="0" width="0" style="display:none;visibility:hidden"></iframe></noscript>
<script>(function(w,d,s,l,i){w[l]=w[l]||[];w[l].push({'gtm.start':
new Date().getTime(),event:'gtm.js'});var f=d.getElementsByTagName(s)[0],
j=d.createElement(s),dl=l!='dataLayer'?'&l='+l:'';j.async=true;j.src=
'//www.googletagmanager.com/gtm.js?id='+i+dl;f.parentNode.insertBefore(j,f);
})(window,document,'script','dataLayer','GTM-KXQ5KL');</script>
<!-- End Google Tag Manager -->

	<?php } // analytics_code()

	/**
	 * Changes the final breadcrumbs for author archives to just the author's name
	 *
	 * Gets the current author info, then removes the final breadcrumb from the crumbs.
	 * Chnages the text to just the author's name.
	 *
	 * If the referring page was from the team page, the "Blog" breadcrumb is removed
	 * from the crumbs. Creates a replacment breadcrumbs linking to the "Team" page and
	 * adds it to the crumbs.
	 *
	 * Adds the Author breadcrumb back to the crumbs, then returns the crumbs.
	 *
	 * @param 	array 		$crumbs 			The current breadcrumbs
	 * @return 	array 							The modified breadcrumbs
	 */
	public function author_archives( $crumbs = array() ) {
		...
		if ( is_author() ) {
			...
			$end 			= array_pop( $crumbs );
			$end['text'] 	= $user->data->display_name;

			if ( strpos( $referer, '/team/' ) !== false ) {

				$blog_link 		= array_pop( $crumbs );
				$team_page 		= get_page_by_title( 'Team' );
				$team_link 		= get_permalink( $team_page->ID );
				$team 			= array( 'text' => 'team', 'url' => $team_link, 'allow_html' => 1 );

				array_push( $crumbs, $team );

			}

			array_push( $crumbs, $end );
	...
	public function background_images() {
		...
		$bgimage = $this->get_thumbnail_url( $pageID, 'full' );

		if ( is_singular( 'post' ) || is_post_type_archive( 'post' ) || is_archive() ) {

			$page 		= get_page_by_title( 'Blog' );
			$bgimage 	= $this->get_thumbnail_url( $page->ID, 'full' );
		...
		if ( ! $bgimage ) {
			...
			$bgimage = get_theme_mod( 'dcc_default_bg' );
		...
		if ( ! is_front_page() ) {
			...
			?><style>
				@media only screen and (min-width: 768px) {
					.site {
						background: linear-gradient( rgba(0, 0, 0, 0.75), rgba(0, 0, 0, 0.75) ), url( <?php echo esc_url( $bgimage ); ?> ) no-repeat 235px 0px fixed;
	...
	public function breadcrumbs() {
		...
		if ( function_exists( 'woocommerce_breadcrumb' ) ) {
			...
			$args['delimiter'] 		= '&nbsp;>&nbsp;';
			$args['home'] 			= esc_html_x( 'Home', 'breadcrumb', 'dcc-2015' );
	...
	public function excerpt_length( $length ) {
		...
		} elseif ( is_front_page() || is_author() ) {
	...
	public function fonts_url() {
		...
		$return 	= '';
		$families 	= '';
		$fonts[] 	= array( 'font' => 'Oxygen', 'weights' => '400,700', 'translate' => esc_html_x( 'on', 'Oxygen font: on or off', 'dcc-2015' ) );
 ...
 */
$dcc_2015_themekit = new dcc_2015_Themekit();
