...
function dcc_2015_analytics_code() { ?>

	<!-- paste code here -->

<?php } // dcc_2015_analytics_code(0)
add_action( 'after_body', 'dcc_2015_analytics_code' );
 ...
 */
function dcc_2015_fonts_url() {

	$return 	= '';
	$families 	= '';
	$fonts[] 	= array( 'font' => 'Oxygen', 'weights' => '400,700', 'translate' => _x( 'on', 'Oxygen font: on or off', 'dcc-2015' ) );
 ...
 */
function dcc_2015_get_posts( $post, $params = array() ) {

	$return = '';
	$return = wp_cache_get( 'dcc_2015_' . $post . '_posts', 'dcc_2015_posts' );
	...
	if ( false === $return ) {
		...
		if ( ! is_wp_error( $query ) && $query->have_posts() ) {
			...
			wp_cache_set( 'dcc_2015_' . $post . '_posts', $query, 'dcc_2015_posts', 5 * MINUTE_IN_SECONDS );
 ...
 */
function dcc_2015_get_svg( $svg, $link = '' ) {
	...
	switch ( $svg ) {
		...
		case 'caret-left' 		: $output .= '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 100 100" class="caret-left"><path d="M76.4 92.6c0 3.3-2.8 6.1-6.1 6.1-1.6 0-3.1-.7-4.3-1.8L23.5 54.4c-1.1-1.1-1.8-2.7-1.8-4.3 0-1.6.7-3.1 1.8-4.3L66 3.3c1.1-1.1 2.7-1.8 4.3-1.8 3.3 0 6.1 2.8 6.1 6.1v85z"/></svg>'; break;
		...
		case 'email' 			: $output .= '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 12.5 100 75" class="email "aria-labelledby="title" role="link"><title id="title">Contact Us Link</title>' . $link . '<path d="M96.5 27.5h-55c-1.7 0-3.4 1.4-3.8 3.1l-7.8 38c-.4 1.7.8 3.1 2.5 3.1h55c1.7 0 3.4-1.4 3.8-3.1l7.8-38c.4-1.7-.7-3.1-2.5-3.1zm-6.6 12.7L65.5 52c-1.1.5-2.3.5-3.1 0L42.9 40.2c-1.4-.9-1.6-2.8-.4-4.4.8-1.1 2-1.7 3.3-1.7.5 0 1 .1 1.5.4l18.2 10.9 22.5-11c.5-.3 1.1-.4 1.6-.4 1.2 0 2.2.7 2.5 1.7.6 1.6-.4 3.6-2.2 4.5zM9.3 35.8H33c1.3 0 2.3-1 2.3-2.3s-1-2.3-2.3-2.3H9.3c-1.3 0-2.3 1-2.3 2.3s1 2.3 2.3 2.3zm17.3 16.1H2.9c-1.3 0-2.3 1-2.3 2.3s1 2.3 2.3 2.3h23.7c1.3 0 2.3-1 2.3-2.3s-1-2.3-2.3-2.3zm5.5-8.2c0-1.3-1-2.3-2.3-2.3H15.6c-1.3 0-2.3 1-2.3 2.3s1 2.3 2.3 2.3h14.2c1.3 0 2.3-1 2.3-2.3zm-8.6 18.4h-7.6c-1.3 0-2.3 1-2.3 2.3s1 2.3 2.3 2.3h7.6c1.3 0 2.3-1 2.3-2.3-.1-1.3-1.1-2.3-2.3-2.3z"/></svg>'; break;
 ...
 * @return 	mixed 					The SVG code
 */
function dcc_2015_the_svg( $svg ) {

	echo dcc_2015_get_svg( $svg );
...
/**
 * [dcc_2015_get_homeposts description]
 * @return [type] [description]
 */
function dcc_2015_get_homeposts() {
 ...
 * @return [type] [description]
 */
function dcc_2015_print_to_footer() {
	...
	$bgimage = dcc_2015_get_featured_url( $pageID, 'full' );

	if ( is_singular( 'post' ) || is_post_type_archive( 'post' ) || is_archive() ) {

		$page 		= get_page_by_title( 'Blog' );
		$bgimage 	= dcc_2015_get_featured_url( $page->ID, 'full' );
	...
	if ( ! $bgimage ) {
		...
		$bgimage = esc_url( get_theme_mod( 'dcc_default_bg' ) );
	...
	if ( ! is_front_page() ) {
		...
		?><style>
			@media only screen and (min-width: 768px) {
				.site {
					background: linear-gradient( rgba(0, 0, 0, 0.75), rgba(0, 0, 0, 0.75) ), url( <?php echo $bgimage; ?> ) no-repeat 235px 0px fixed;
...
} // dcc_2015_print_to_footer()
add_action( 'wp_footer', 'dcc_2015_print_to_footer' );

/**
 * Turn off comments for attachments
...
/**
 * Changes excerpt length on blog pages
 *
 * @param  [type] $length [description]
 * @return [type]         [description]
 */
function dcc_blog_page_excerpt_length( $length ) {
...
add_action( 'excerpt_length',  'dcc_blog_page_excerpt_length'  );

/**
 * Custom "read more" link
 * @return [type] [description]
 */
function dcc_read_more_link() {

	return '<a class="read-more" href="' . get_permalink() . '">Read More</a>';

}
add_filter( 'the_content_more_link', 'dcc_read_more_link' );
 ...
 * @return [type]       [description]
 */
function dcc_new_excerpt_more( $more ) {

	return '...';

}
add_filter('excerpt_more', 'dcc_new_excerpt_more');
 ...
 */
function dcc_2015_get_featured_url( $postID, $size = 'thumbnail' ) {
 ...
 */
function dcc_2015_shorten_text( $text, $limit = 100, $after = '...' ) {
 ...
 */
function dcc_2015_add_mime_types( $post_mime_types ) {

    $post_mime_types['application/pdf'] = array( esc_html__( 'PDFs', 'dcc-2015' ), esc_html__( 'Manage PDFs', 'dcc-2015' ), _n_noop( 'PDF <span class="count">(%s)</span>', 'PDFs <span class="count">(%s)</span>' ) );
    $post_mime_types['text/x-vcard'] = array( esc_html__( 'vCards', 'dcc-2015' ), esc_html__( 'Manage vCards', 'dcc-2015' ), _n_noop( 'vCard <span class="count">(%s)</span>', 'vCards <span class="count">(%s)</span>' ) );

    return $post_mime_types;

} // dcc_2015_add_mime_types

add_filter( 'post_mime_types', 'dcc_2015_add_mime_types' );
 ...
 * @return [type]                 [description]
 */
function dcc_2015_custom_upload_mimes( $existing_mimes = array() ) {
...
} // dcc_2015_custom_upload_mimes()

add_filter( 'upload_mimes', 'dcc_2015_custom_upload_mimes' );
 ...
 */
function dcc_2015_page_body_classes( $classes ) {
...
} // dcc_2015_page_body_classes()
add_filter( 'body_class', 'dcc_2015_page_body_classes' );
 ...
 */
function dcc_2015_add_favicons() {
...
add_action( 'wp_head', 'dcc_2015_add_favicons' );
 ...
 */
function dcc_2015_make_number( $number ) {
 ...
 * @return 	void
 */
function dcc_2015_customize_login_page() {

	wp_enqueue_style( 'scriptname-login', get_stylesheet_directory_uri() . '/login.css' );

} // dcc_2015_customize_login_page()
add_action( 'login_enqueue_scripts', 'dcc_2015_customize_login_page' );
